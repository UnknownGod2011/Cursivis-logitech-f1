...
function buildShoppingFallbackPlan({ originalText, resultText, voiceCommand }) {
  ...
  return {
    ...
    summary: `Open a new tab, search for ${query}, and continue toward add-to-cart flow with confirmation.`,
...
export function createBrowserActionPlanner({ generateText }) {
  return async ({
    ...
    const directFormsPlan = isFormsLike({ taskPack, contentType, voiceCommand })
      ? buildFormsFallbackPlan({ originalText, resultText, voiceCommand, browserContext })
      : null;
    if (directFormsPlan) {
    ...
    const directShoppingPlan = isShoppingLike({
      ...
      ? buildShoppingFallbackPlan({ originalText, resultText, voiceCommand })
      : null;
    ...
    const directTaskPackPlan =
      (taskPack?.id === "mail_compose"
        ? buildMailFallbackPlan({ browserContext, originalText, resultText, voiceCommand })
        : null) ||
      (taskPack?.id === "discord"
        ? buildDiscordFallbackPlan({ resultText, voiceCommand, browserContext })
        : null) ||
      (taskPack?.id === "shopping"
        ? buildShoppingFallbackPlan({ originalText, resultText, voiceCommand })
        : null) ||
      ((taskPack?.id === "google_docs" || taskPack?.id === "notion")
        ? buildEditorFallbackPlan({ resultText, voiceCommand, browserContext, taskPack })
        : null);
